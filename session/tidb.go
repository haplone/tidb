...
func (dm *domainMap) Get(store kv.Storage) (d *domain.Domain, err error) {
	...
	err = util.RunWithRetry(util.DefaultMaxRetries, util.RetryInterval, func() (retry bool, err1 error) {
		log.Infof("new Domain with store %v new domain, ddl lease %v, stats lease %d", store.UUID(), ddlLease, statisticLease)
...
func runStmt(ctx context.Context, sctx sessionctx.Context, s ast.Statement) (ast.RecordSet, error) {
	...
	se := sctx.(*session)
	log.Infof("convert sessionctx.Context -> session")
	rs, err = s.Exec(ctx)
